...
impl std::fmt::Debug for PrecompFuns {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    f.debug_struct("PrecompFuns")
      .field("visit", &"VisitFun")
...
impl Clone for Precomp {
  fn clone(&self) -> Self {
    ...
    Precomp { id: self.id, name: self.name, smap: self.smap, funs }
  }
}
...
lazy_static! {
  pub static ref PRECOMP: Arc<Box<Vec<Precomp>>> = {
    let mut precomp = Box::new(default_precomp());
    Arc::new(precomp)
  };
}

pub fn default_precomp() -> Vec<Precomp> {
  vec![
    Precomp {
      id: STRING_NIL,
      name: "String.nil",
