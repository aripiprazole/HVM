...
pub type VisitFun = Arc<dyn Fn(ReduceCtx) -> bool + Send + Sync + 'static>;
pub type ApplyFun = Arc<dyn Fn(ReduceCtx) -> bool + Send + Sync + 'static>;
        ...
        if let Some(fs) = &precomp.funs {
          funs.insert(
            ...
            Function::Compiled {
              ...
              visit: Arc::clone(&fs.visit),
              apply: Arc::clone(&fs.apply),
            },
          );
        }
